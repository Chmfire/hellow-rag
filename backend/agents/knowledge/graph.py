...
from .nodes import (
    ...
    select_top_k_chunks,
    generate_answer,
    hallucination_check_and_citation as hallucination_check,
    check_quality,
    finalize_metrics,
)
...
def should_check_hallucination(state: KnowledgeAgentState) -> Literal["hallucination_check", "finalize_metrics"]:
    """根据配置决定是否进行幻觉检测（流式模式可关闭以加速）"""
    config = state["config"]
    if getattr(config, "enable_hallucination_check", True):
