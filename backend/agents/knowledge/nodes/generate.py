...
def prepare_generation_context(state: KnowledgeAgentState, config=None) -> Dict[str, Any]:
    ...
    # 调试日志：检查 context 是否为空
    logger.info(f"[GenerateContext] unique_vector_chunks={len(unique_vector_chunks)}, vector_context_text_len={len(vector_context_text)}")
    if vector_context_text:
        preview = vector_context_text[:300]
        logger.info(f"[GenerateContext] context_preview: {preview}...")
    else:
        logger.warning("[GenerateContext] vector_context_text 为空！LLM 将收到空知识库！")
...
def _finalize_generate_outputs(
    ...
    # 置信度 = 检索相关度(40%) + 答案与检索内容重叠度(60%)
    retrieval_score = 0.0
    if reranked_chunks:
        def _s(c):
            return (c.get("score", 0) if isinstance(c, dict) else getattr(c, "score", 0)) or 0
        top_scores = [_s(c) for c in reranked_chunks[:3]]
        retrieval_score = sum(top_scores) / len(top_scores) if top_scores else 0.0

    # 计算答案与检索内容的关键字重叠度（Jaccard）
    content_overlap = 0.0
    if reranked_chunks and answer:
        import re
        # 提取答案中的实词（中文2字以上，英文字母数字2字符以上）
        answer_words = set(re.findall(r'[\u4e00-\u9fa5]{2,}|[a-zA-Z0-9]{2,}', answer.lower()))
        if answer_words:
            # 合并所有检索chunk的内容
            all_context_words = set()
            for chunk in reranked_chunks[:5]:  # 只看前5个最相关的chunk
                content = chunk.get('content', '') if isinstance(chunk, dict) else getattr(chunk, 'content', '') or ''
                all_context_words.update(re.findall(r'[\u4e00-\u9fa5]{2,}|[a-zA-Z0-9]{2,}', content.lower()))
            
            if all_context_words:
                intersection = answer_words & all_context_words
                content_overlap = len(intersection) / len(answer_words)  # 召回率视角

    # 综合置信度：检索分(40%) + 内容重叠(60%)
    confidence = 0.4 * min(retrieval_score, 1.0) + 0.6 * min(content_overlap, 1.0)
    confidence = round(min(max(confidence, 0.0), 1.0), 3)  # 限制在[0,1]区间，保留3位小数
