...
async def _async_persist_conversation_messages(
    session_id: str,
    query: str,
    answer_text: str,
    sources: list,
    ...
    """异步版本的持久化，在线程池中执行，不阻塞 SSE 流关闭"""
    import asyncio
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(
        None,
        _persist_conversation_messages,
        session_id, query, answer_text, sources, confidence, query_image_oss_key,
    )
...
async def stream_knowledge_qa_sse(
    ...
    initial_state = create_initial_state(
        ...
        config=RAGConfig(
            ...
            enable_citations=True,
            enable_fallback=True,
            enable_hallucination_check=False,  # 流式模式关闭幻觉检测以加速
            collection=collection or None,
            ranker=rc.get("ranker", "RRF"),
            rrf_k=rc.get("rrf_k", 60),
            hybrid_alpha=rc.get("hybrid_alpha", 0.5),
    ...
    # 发送思考链路初始化事件
    yield _sse("thinking", {"step": "initializing", "message": "正在初始化知识库查询..."})

    try:
        # 发送思考链路事件：正在分析问题
        yield _sse("thinking", {"step": "analyzing", "message": "正在分析问题..."})
    ...
    try:
        # 发送思考链路事件：正在检索知识
        yield _sse("thinking", {"step": "retrieving", "message": "正在检索相关知识..."})
    ...
    # 发送思考链路事件：正在准备回答
    yield _sse("thinking", {"step": "preparing", "message": "正在准备生成回答..."})
    ...
    try:
        # 发送思考链路事件：正在生成回答
        yield _sse("thinking", {"step": "generating", "message": "正在生成回答..."})
        async for piece in iter_openai_text_deltas(ctx["messages"], ctx["model_name"]):
            # 检查是否收到停止事件
            if stop_event and stop_event.is_set():
                logger.info(f"[Stream] 收到停止信号，中断生成")
                yield _sse("stopped", {"message": "用户中断了回答生成"})
                break
    ...
    # 持久化在 done 之后异步执行，不阻塞 SSE 流关闭
    import asyncio
    asyncio.create_task(
        _async_persist_conversation_messages(
            session_id,
            query,
            answer_final,
