...
class KGRetrievalService:
    ...
    def _post(self, path: str, json: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"[KGRetrieval] POST 请求超时: {url}")
            raise
        except httpx.ConnectError:
            logger.warning(f"[KGRetrieval] POST 连接失败: {url}")
            raise
        except httpx.HTTPStatusError as e:
            logger.warning(f"[KGRetrieval] POST HTTP 错误: {e}")
            raise
        except Exception as e:
            logger.warning(f"[KGRetrieval] POST 请求失败: {e}")
            raise
