...
async def upload_document(
    ...
    1. 校验文件
    2. 计算文件哈希
    3. 检查相同哈希的文件是否已存在，存在则拒绝（要求用户先删除旧版本）
    4. 上传 OSS（kb/{kb_name}/{file_name}）
    5. 关联到 __default__ 类目，写 knowledge_category_file
    6. 写 knowledge_file + knowledge_job(pending)
    7. 触发后台任务（切分）
    8. 立即返回 job_id
    """
    ...
    # 检查相同哈希的文件是否已存在，存在则拒绝覆盖
    existing_by_hash = file_repo.get_by_kb_and_file_hash(kb["id"], file_hash)
    if existing_by_hash:
        raise ConflictError(
            f"内容相同的文件「{existing_by_hash['file_name']}」已存在于知识库「{kb_name}」，请先在文件列表删除旧版本后重新上传"
        )

    # 检查同名文件是否已存在（额外检查，防止哈希冲突）
    existing_by_name = file_repo.get_by_kb_and_oss_key(kb["id"], oss_key)
    if existing_by_name:
        raise ConflictError(
...
async def start_chunking(
    ...
    for f in all_files:
        ...
        try:
            ...
            # 对于类目文件，我们无法直接获取文件内容计算哈希
            # 这里暂时只检查文件名，因为类目文件的哈希检查需要额外的逻辑
            # 后续可以考虑从 OSS 下载文件计算哈希，但会增加网络开销

            file_record = file_repo.create(
                kb_id=kb["id"],
                file_name=file_name,
                oss_key=oss_key,
                file_hash=None,  # 类目文件暂时无法计算哈希
