...
同时支持三级分块模式（用于 Auto-merging Retriever）
"""
import re
...
def split_text(
    ...
    如果 use_three_level=True，将使用三级分块模式（用于 Auto-merging Retriever）
    """
    if not text or not text.strip():
        return []
    
    if use_three_level:
        splitter = ThreeLevelChunkSplitter(chunk_size, chunk_overlap)
        chunks = splitter.split_text_with_metadata(text, metadata, chunk_size, chunk_overlap)
...
def split_text_with_metadata(
    ...
    if use_three_level:
        splitter = ThreeLevelChunkSplitter(chunk_size, chunk_overlap)
        return splitter.split_text_with_metadata(text, base_metadata, chunk_size, chunk_overlap)
