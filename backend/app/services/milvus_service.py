...
class MilvusService:
    ...
    def get_or_create_collection(
        ...
        schema.add_field("chunk_id",    DataType.VARCHAR, max_length=36, is_primary_key=True)
        ...
        schema.add_field("parent_chunk_id", DataType.VARCHAR, max_length=36)
        schema.add_field(
    ...
    def upsert_chunks(
        ...
        for chunk, vec, indexed_content in zip(chunks, vectors, texts):
            row: Dict[str, Any] = {
                ...
                "chunk_level":     int(chunk.get("chunk_level", 0)),
    ...
    def _analyze_query(self, query: str, query_type: Optional[str] = None) -> Dict[str, Any]:
        ...
        has_keywords = len(query.split()) > 5

        # 优先使用外部传入的 query_type
        if query_type:
            if query_type == 'code':
                # 代码查询，更依赖关键词
                return {
                    'hybrid_alpha': 0.2,  # 更重视 BM25
                    'rrf_k': 30,          # 更集中的排序
                    'top_k': 15           # 更多候选
                }
    ...
    def hybrid_search(
        ...
        query_type: 查询类型，用于优化检索参数。
        """
        # 分析查询，动态调整参数
        query_params = self._analyze_query(query, query_type)
        # 使用动态参数，但保留用户指定的参数优先级
        if rrf_k == 60:  # 只在使用默认值时才动态调整
            rrf_k = query_params['rrf_k']
        if hybrid_alpha == 0.5:  # 只在使用默认值时才动态调整
            hybrid_alpha = query_params['hybrid_alpha']
        if top_k == 10:  # 只在使用默认值时才动态调整
            top_k = query_params['top_k']

        logger.info(f"[Milvus] 使用动态检索参数: hybrid_alpha={hybrid_alpha}, rrf_k={rrf_k}, top_k={top_k}, query_type={query_type}")
        ...
        output_fields = ["chunk_id", "job_id", "file_name", "chunk_index", "content", "chunk_level", "parent_chunk_id"]
        ...
        for hit in (results[0] if results else []):
            ...
            hits.append({
                ...
                "metadata":        {
                    ...
                    if k not in ("chunk_id", "job_id", "file_name", "chunk_index", "content", "chunk_level", "parent_chunk_id")
                },
            })
