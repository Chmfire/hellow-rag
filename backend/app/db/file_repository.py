...
class FileRepository(BaseRepository):
    ...
    def create(
        ...
        *,
        ...
        rows = self._execute_returning(
            """
            INSERT INTO knowledge_file(kb_id, category_file_id, file_name, oss_key, file_hash, file_size, mime_type, status, sync_graph)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
            RETURNING *
            """,
            (kb_id, category_file_id, file_name, oss_key, file_hash, file_size, mime_type, status, sync_graph),
        )
    ...
    def get_by_kb_and_file_hash(self, kb_id: str, file_hash: str) -> Optional[Dict[str, Any]]:
        rows = self._execute_select(
            "SELECT * FROM knowledge_file WHERE kb_id = %s AND file_hash = %s ORDER BY created_at DESC LIMIT 1",
            (kb_id, file_hash),
        )
        return self._normalize(rows[0]) if rows else None
