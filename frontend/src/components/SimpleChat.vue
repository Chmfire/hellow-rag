<template>
  <div class="chat-wrapper">
      ...
      <transition-group name="msg" tag="div">
        <div v-for="(msg, i) in messages" :key="i" :class="['msg-row', msg.role]">
          <div v-if="msg.role === 'assistant'" class="ai-avatar">
            <div class="ai-avatar-ring" :class="{ 'pulsing': loading }" />
          ...
          <div class="bubble-wrap">
            <div class="bubble" :class="msg.role">
              ...
              <!-- 断点续传：网络中断后显示重试按钮 -->
              <div v-if="msg._interrupted" class="interrupt-recovery">
                <div class="interrupt-info">
                  <svg viewBox="0 0 16 16" fill="none" class="interrupt-icon">
                    <circle cx="8" cy="8" r="6" stroke="currentColor" stroke-width="1.3"/>
                    <line x1="8" y1="4" x2="8" y2="8" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/>
                    <line x1="8" y1="8" x2="11" y2="9.5" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/>
                  </svg>
                  <span class="interrupt-text">网络中断，已保存 {{ msg._savedLength || 0 }} 字</span>
                </div>
                <button class="resume-btn" @click="resumeGeneration(msg)" :disabled="msg._resuming">
                  <svg v-if="msg._resuming" viewBox="0 0 16 16" fill="none" class="resume-spinner">
                    <circle cx="8" cy="8" r="6" stroke="currentColor" stroke-width="1.5" stroke-dasharray="28" stroke-dashoffset="10"/>
                  </svg>
                  <svg v-else viewBox="0 0 16 16" fill="none" class="resume-icon">
                    <path d="M3 2v6l4-3-4-3zm10 12V8l-4 3 4 3z" fill="currentColor"/>
                  </svg>
                  <span>{{ msg._resuming ? '正在恢复...' : '恢复生成' }}</span>
                </button>
                <button class="retry-btn" @click="retryGeneration(msg)">
                  <svg viewBox="0 0 16 16" fill="none" class="retry-icon">
                    <path d="M13.5 8a5.5 5.5 0 11-1.7-4" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/>
                    <path d="M11 1h3v3" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                  <span>重新生成</span>
                </button>
              </div>
    ...
    <div class="input-area" :class="{ focused: inputFocused }">
      ...
      <!-- 停止按钮（仅在加载状态下显示） -->
      <template v-if="loading && chatMode === 'knowledge'">
    ...
    if (chatMode.value === 'knowledge') {
      ...
      await apiService.knowledgeQueryStream(
        ...
        },
        {
          ...
          onThinking: (t) => {
            const row = messages.value[assistantIdx]
            if (row && row.thinkingSteps) {
              row.thinkingSteps.push(t)
            }
          },
          onDelta: (d) => {
            streamRaw += d.text || ''
...
const stopGeneration = async () => {
  if (!loading.value || chatMode.value !== 'knowledge') return
  try {
    await apiService.stopKnowledgeQA(currentSessionId.value || 'default')
  } catch (e) {
...
/**
 * 恢复生成（断点续传）
 * 从后端获取已保存的内容，继续生成剩余部分
 */
const resumeGeneration = async (msg) => {
  if (msg._resuming) return
  ...
  try {
    // 1. 从后端获取已保存的进度
    const resumeData = await apiService.getStreamResume(currentSessionId.value)
    
    if (!resumeData.can_resume) {
      ElMessage.warning(resumeData.message || '无法恢复生成')
    ...
    // 2. 显示已保存的内容
    let streamRaw = resumeData.content || ''
    msg.content = md.render(streamRaw)
    msg._savedLength = streamRaw.length
    
    ElMessage.success(`已恢复 ${streamRaw.length} 字，继续生成...`)
    
    // 3. 重新发起请求，继续生成
    loading.value = true
    const assistantIdx = messages.value.indexOf(msg)
    let imageMap = {}
    
    await apiService.knowledgeQueryStream(
      {
        query: messages.value[messages.value.indexOf(msg) - 1]?.content || '',
      ...
      },
      {
        ...
        onDone: (d) => {
          ...
          msg.sources = d.sources?.length ? [...d.sources] : msg.sources
          msg._resuming = false
          msg._interrupted = false
 ...
 * 重新生成（删除当前消息，重新发起请求）
 */
const retryGeneration = async (msg) => {
  // 获取用户问题（当前消息的前一条）
  const userMsgIdx = messages.value.indexOf(msg) - 1
  if (userMsgIdx < 0 || messages.value[userMsgIdx]?.role !== 'user') {
    ElMessage.error('未找到用户问题')
    return
  }
...
.ai-avatar-ring.pulsing {
  ...
  animation: orb-spin 5s linear infinite, ring-pulse 1.2s ease-in-out infinite;
...
.thinking-step.initializing .thinking-dot { background: #a78bfa; box-shadow: 0 0 8px rgba(167,139,250,0.6); }
.thinking-step.analyzing .thinking-dot { background: #f5c842; box-shadow: 0 0 8px rgba(245,200,66,0.6); }
.thinking-step.retrieving .thinking-dot { background: #2dd4a0; box-shadow: 0 0 8px rgba(45,212,160,0.6); }
.thinking-step.preparing .thinking-dot { background: #7c3aed; box-shadow: 0 0 8px rgba(124,58,237,0.6); }
.thinking-step.generating .thinking-dot { background: #3b6fd4; box-shadow: 0 0 8px rgba(59,111,212,0.6); animation: pulse 1.5s ease-in-out infinite; }
@keyframes pulse {
...
.stop-btn {
  ...
  background: linear-gradient(135deg, #d43b3b, #ed3a3a);
  color: #fff; display: flex; align-items: center; justify-content: center;
  flex-shrink: 0; transition: all 0.25s cubic-bezier(0.4,0,0.2,1);
  box-shadow: 0 2px 16px rgba(212,59,59,0.5), 0 0 0 1px rgba(255,255,255,0.08) inset;
  position: relative; overflow: hidden;
}
.stop-btn::before {
  content: '';
...
.stop-btn:hover {
  box-shadow: 0 6px 28px rgba(212,59,59,0.7), 0 0 0 3px rgba(237,58,58,0.25), 0 0 0 1px rgba(255,255,255,0.1) inset;
  transform: translateY(-2px) scale(1.04);
}
.stop-btn:active { transform: translateY(0) scale(0.97); }
